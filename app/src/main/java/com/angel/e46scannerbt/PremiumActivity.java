...
public class PremiumActivity extends Activity {
    ...
    private void showBackup() { base("Backup seguro", true, true, 1); info("Obligatorio antes de pruebas avanzadas. Todo READ ONLY."); section("CHECKLIST"); checklistBox=card(checklist(),13); body.addView(checklistBox); section("ACCIONES"); body.addView(action("1 · Conectar Bluetooth",this::realConnect)); body.addView(action("2 · Inicializar ELM327",this::realElm)); body.addView(action("3 · Backup OBD/ECU READ ONLY",this::realBackup)); body.addView(action("4 · Compartir backup completo",this::share)); logSection(); }

    private void showTests() { base("Pruebas", true, true, 1); info("Test guiado. Avanzados bloqueados hasta backup."); section("CHECKLIST"); checklistBox=card(checklist(),13); body.addView(checklistBox); section("LECTURA SEGURA"); body.addView(action("Test 1 · Conexión + ELM",()->{realConnect();realElm();})); body.addView(action("Test 2 · Protocolo ISO/KWP",this::realProtocol)); body.addView(action("Test 3 · Motor/PIDs",this::realMotor)); body.addView(action("Test 4 · DTC motor",this::realDtc)); section("PRUEBAS AVANZADAS"); body.addView(action("Test 5 · Preparar LSZ LED",()->guarded("Preparar LSZ LED cold/warm"))); body.addView(action("Test 6 · Preparar GM5 confort",()->guarded("Preparar GM5 comfort close / blink"))); body.addView(action("Exportar sesión",this::share)); logSection(); }
