...
public class PremiumActivity extends Activity {
    ...
    private void showBackup() { base("Backup seguro", true, true, 1); info("Obligatorio antes de pruebas avanzadas. Todo READ ONLY."); section("CHECKLIST"); checklistBox=card(checklist(),13); body.addView(checklistBox); section("ACCIONES"); body.addView(action("1 · Conectar Bluetooth",this::realConnect)); body.addView(action("2 · Inicializar ELM327",this::realElm)); body.addView(action("3 · Backup OBD/ECU READ ONLY",this::realBackup)); body.addView(action("4 · Leer LSZ luces READ ONLY",()->realModule("LSZ"))); body.addView(action("5 · Leer GM5 confort READ ONLY",()->realModule("GM5"))); body.addView(action("6 · Compartir backup completo",this::share)); logSection(); }

    private void showTests() { base("Pruebas", true, true, 1); info("Test guiado. Avanzados bloqueados hasta backup. LSZ/GM5 ejecutan lectura BMW sin escritura."); section("CHECKLIST"); checklistBox=card(checklist(),13); body.addView(checklistBox); section("LECTURA SEGURA"); body.addView(action("Test 1 · Conexión + ELM",()->{realConnect();realElm();})); body.addView(action("Test 2 · Protocolo ISO/KWP",this::realProtocol)); body.addView(action("Test 3 · Motor/PIDs",this::realMotor)); body.addView(action("Test 4 · DTC motor",this::realDtc)); section("MODULOS BMW"); body.addView(action("Test 5 · Init BMW KWP",this::realBmwKwp)); body.addView(action("Test 6 · Leer LSZ / LCM luces",()->realModule("LSZ"))); body.addView(action("Test 7 · Leer GM5 / ZKE confort",()->realModule("GM5"))); section("WRITE PLAN BLOQUEADO"); body.addView(action("Plan LSZ · LED cold/warm check",()->addLog(elmClient.buildWritePlan("LSZ","LED cold/warm check")))); body.addView(action("Plan GM5 · comfort close / blink",()->addLog(elmClient.buildWritePlan("GM5","comfort close / blink")))); body.addView(action("Exportar sesión",this::share)); logSection(); }
