...
public class MainActivity extends Activity {
    ...
    boolean btOk=false, elmOk=false, protocolOk=false, engineOk=false, dtcOk=false;
    static final UUID SPP = UUID.fromString("00001101-0000-1000-8000-00805F9B34FB");
    final int BG=Color.rgb(2,6,12), CARD=Color.rgb(12,19,29), BLUE=Color.rgb(0,122,255), TXT=Color.WHITE, MUTED=Color.rgb(156,164,176), GREEN=Color.rgb(40,220,120), YELLOW=Color.rgb(245,190,60), RED=Color.rgb(255,75,85);
    ...
    void base(String title){ main=new LinearLayout(this); main.setOrientation(LinearLayout.VERTICAL); main.setPadding(24,46,24,10); main.setBackgroundColor(BG); TextView top=tv("☰     "+title+" ",21,TXT,false); top.append("E46"); status=tv(stateText(),14,Color.rgb(110,200,255),false); main.addView(top); main.addView(status); setContentView(main); }
    String stateText(){ return "↯ "+lastTest+"   BT "+(btOk?"OK":"OFF")+"   ELM "+(elmOk?"OK":"--")+"   "+proto+"   "+volts; }
    String flag(boolean b){ return b?"✅":"⬜"; }
    String testsText(){ return flag(btOk)+" Bluetooth   "+flag(elmOk)+" ELM   "+flag(protocolOk)+" Protocolo\n"+flag(engineOk)+" Motor/PIDs   "+flag(dtcOk)+" DTC   "+(lastDtc.contains("P0401")?"⚠️ P0401":"✅ Sin alerta nueva"); }
    ...
    void showHome(){ base("Coding Lab"); main.addView(hero()); testPanel=panel(testsText(),14); main.addView(testPanel); main.addView(section("MENÚ PRINCIPAL")); main.addView(menu("🔧", "Coding Lab", "Funciones de confort y personalización", () -> showCoding())); main.addView(menu("💡", "Luces", "Iluminación exterior e interior", () -> showLights())); main.addView(menu("▭", "Ventanillas", "Funciones de confort de ventanas", () -> showWindows())); main.addView(menu("🟢", "Diagnóstico", "Leer errores y estado de módulos", () -> showDiag())); main.addView(menu("ⓘ", "Información del coche", "Detalles del vehículo y módulos", () -> showInfo())); main.addView(menu("▤", "Logs", "Registros y sesiones guardadas", () -> showLogs())); main.addView(nav()); }
    TextView hero(){ TextView h=panel("✓ Conectado\nBMW E46 320d M47N\nVIN: pendiente\n\n        ▄▄▄ BMW E46 COUPÉ ▄▄▄\n\n"+liveText(),15); h.setTextColor(Color.rgb(235,240,248)); live=h; return h; }
