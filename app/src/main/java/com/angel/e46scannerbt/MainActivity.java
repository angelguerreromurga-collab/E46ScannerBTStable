...
public class MainActivity extends Activity {
    ...
    final int BG = Color.rgb(1, 5, 12);
    final int CARD = Color.rgb(7, 16, 29);
    final int CARD2 = Color.rgb(10, 20, 35);
    final int LINE = Color.rgb(24, 50, 78);
    final int TXT = Color.WHITE;
    final int MUTED = Color.rgb(157, 169, 187);
    final int BLUE = Color.rgb(0, 122, 255);
    final int BLUE_DARK = Color.rgb(0, 70, 180);
    final int GREEN = Color.rgb(35, 220, 115);
    final int RED = Color.rgb(255, 58, 72);
    final int YELLOW = Color.rgb(245, 198, 45);
    final int PURPLE = Color.rgb(146, 86, 255);
    ...
    GradientDrawable screenBg() {
        return new GradientDrawable(GradientDrawable.Orientation.TOP_BOTTOM,
                new int[]{Color.rgb(2, 10, 20), Color.rgb(0, 5, 11)});
    }
    ...
    void base(String title, boolean back, boolean bottomNav, boolean codingButtons, int tab) {
        ...
        body.setPadding(dp(20), dp(12), dp(20), dp(4));
        scroll.addView(body);
    ...
    void header(String title, boolean back) {
        ...
        h.addView(left, new LinearLayout.LayoutParams(dp(42), dp(42)));

        TextView name = txt(title, 20, TXT, false);
        name.setGravity(Gravity.CENTER);
        h.addView(name, new LinearLayout.LayoutParams(0, dp(42), 1));

        TextView right = txt(back ? "▱" : "V3.0", back ? 24 : 12, back ? TXT : BLUE, true);
        right.setGravity(Gravity.CENTER);
        h.addView(right, new LinearLayout.LayoutParams(dp(50), dp(42)));
        body.addView(h);
    ...
    public void showHome() {
        ...
        LinearLayout.LayoutParams hp = new LinearLayout.LayoutParams(-1, dp(205));
        hp.setMargins(0, 0, 0, dp(2));
        hero.setLayoutParams(hp);

        ImageView glow = new ImageView(this);
        glow.setBackground(makeGlow());
        FrameLayout.LayoutParams gp = new FrameLayout.LayoutParams(dp(260), dp(72), Gravity.RIGHT | Gravity.BOTTOM);
        gp.rightMargin = dp(20);
        gp.bottomMargin = dp(8);
        hero.addView(glow, gp);

        ImageView car = new ImageView(this);
        Bitmap b = loadHeroBitmap();
        if (b != null) car.setImageBitmap(b);
        else car.setImageResource(getResources().getIdentifier("bmw_e46_black_coupe_hero", "drawable", getPackageName()));
        car.setScaleType(ImageView.ScaleType.FIT_CENTER);
        car.setAdjustViewBounds(true);
        FrameLayout.LayoutParams cp = new FrameLayout.LayoutParams(dp(360), dp(205), Gravity.RIGHT | Gravity.CENTER_VERTICAL);
        ...
        info.addView(txt("VIN: " + vin, 12, Color.rgb(205, 212, 224), false));
        TextView mode = txt("READ ONLY · SAFE MODE", 10, BLUE, true);
        mode.setPadding(0, dp(4), 0, 0);
        info.addView(mode);
        FrameLayout.LayoutParams ip = new FrameLayout.LayoutParams(dp(185), -1, Gravity.LEFT | Gravity.CENTER_VERTICAL);
        hero.addView(info, ip);
        body.addView(hero);
    ...
    GradientDrawable makeGlow() {
        GradientDrawable g = new GradientDrawable(GradientDrawable.Orientation.LEFT_RIGHT,
                new int[]{Color.TRANSPARENT, Color.argb(100, 0, 122, 255), Color.TRANSPARENT});
        g.setCornerRadius(dp(40));
        return g;
    }

    Bitmap loadHeroBitmap() {
        try {
            int id = getResources().getIdentifier("e46_hero_car_b64", "raw", getPackageName());
            if (id == 0) return null;
            ...
            byte[] img = Base64.decode(bos.toString("UTF-8").replaceAll("\\s", ""), Base64.DEFAULT);
    ...
    View menuRow(String title, String sub, String icon, int color, final Runnable click) {
        ...
        row.setBackground(bg(CARD2, 12, LINE));
        LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(-1, dp(62));
        lp.setMargins(0, 0, 0, dp(7));
        row.setLayoutParams(lp);

        TextView ic = txt(icon, 27, color, false);
        ic.setGravity(Gravity.CENTER);
        row.addView(ic, new LinearLayout.LayoutParams(dp(50), -1));

        LinearLayout texts = new LinearLayout(this);
        texts.setOrientation(LinearLayout.VERTICAL);
        texts.setGravity(Gravity.CENTER_VERTICAL);
        texts.addView(txt(title, 16, TXT, false));
        texts.addView(txt(sub, 12, Color.rgb(183, 193, 207), false));
        row.addView(texts, new LinearLayout.LayoutParams(0, -1, 1));

        TextView arrow = txt("›", 31, Color.rgb(190, 199, 212), false);
    ...
    void addBottomNav(int active) {
        ...
        nav.setPadding(dp(14), dp(3), dp(14), dp(4));
        nav.setBackgroundColor(Color.rgb(2, 7, 14));
        String[] names = {"⌂\nInicio", "◇\nMódulos", "≡\nLogs", "⚙\nAjustes"};
        ...
        for (int i = 0; i < 4; i++) {
            ...
            nav.addView(v, new LinearLayout.LayoutParams(0, dp(54), 1));
        }
    ...
    void tabs(int active) {
        ...
        for (int i = 0; i < 4; i++) {
            final int idx = i;
            TextView v = txt(a[i], 12, i == active ? BLUE : Color.rgb(215, 220, 230), true);
            v.setGravity(Gravity.CENTER);
            v.setOnClickListener(x -> { if (idx == 0) showLights(); else if (idx == 1) showCoding(); else if (idx == 2) showComfort(); else showInfo(); });
            r.addView(v, new LinearLayout.LayoutParams(0, dp(36), 1));
        }
        body.addView(r);
    ...
    TextView infoCard(String s) {
        TextView v = txt("ⓘ    " + s, 13, Color.rgb(210, 222, 236), false);
        v.setPadding(dp(14), dp(10), dp(14), dp(10));
        v.setBackground(bg(CARD2, 10, LINE));
        return v;
    }
    ...
    LinearLayout group(View[] rows) {
        ...
        g.setBackground(bg(CARD2, 10, LINE));
    ...
    View switchRow(String title, String sub, boolean checked) {
        ...
        TextView desc = txt(sub, 12, Color.rgb(188, 200, 214), false);
        desc.setMaxLines(2);
        texts.addView(desc);
    ...
    void tintSwitch(Switch sw) {
        if (Build.VERSION.SDK_INT >= 21) {
            ...
            sw.setThumbTintList(new ColorStateList(states, new int[]{Color.WHITE, Color.rgb(205, 212, 220)}));
            sw.setTrackTintList(new ColorStateList(states, new int[]{BLUE, Color.rgb(55, 67, 82)}));
        }
    }
    ...
    void addCodingButtons() {
        ...
        bar.setBackgroundColor(Color.rgb(2, 7, 14));
    ...
    TextView bottomButton(String s, boolean primary) {
        ...
        v.setGravity(Gravity.CENTER);
        v.setBackground(bg(primary ? BLUE : Color.TRANSPARENT, 8, primary ? BLUE_DARK : BLUE));
        return v;
    }
